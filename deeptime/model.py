...
class Simulation:
    ...
    def _seed_civilizations(self) -> None:
        ...
        for a in range(n_civ):
            ...
            self.A_dom[a, i, :] = self.x[a, i, S.A] / S.N_DOMAINS
    ...
    def continuous_drift(self, Heff: np.ndarray) -> np.ndarray:
        ...
        dx[..., S.I] = p.g_I * (1.0 + 0.5 * Inorm) - p.r_I * p.redistribution - p.delta_I * I
        ...
        build = (Q * Y) / (1.0 + Q * Y)
        dx[..., S.Z] = (p.g_Z * build * (1.0 - Zz) - p.delta_Z * Zz - p.chi_W * wd * Zz)
    ...
    def _migrate(self, dt: float) -> None:
        ...
        for a in self.alive_indices():
            ...
            # flow[i,j] = m0 * P_i * kernel_ij * attract_j / attract_i
            flow = (p.migrate_m0 * P[:, None] * kernel
                    * (attract[None, :] / (attract[:, None] + TINY))) * dt
    ...
    def _sample_collapse(self, dt: float) -> None:
        ...
        h = p.h0_collapse * np.exp(np.clip(
            ...
            + B_D_COLLAPSE * disruption - p.b_G * x[..., S.G] - p.b_Z * x[..., S.Z], -60, 60))
        prob = 1 - np.exp(-h * dt)
